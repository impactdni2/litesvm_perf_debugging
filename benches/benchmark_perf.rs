...
use agave_feature_set::{FeatureSet, switch_to_new_elf_parser};
...
use solana_sdk::{
  clock::Clock, pubkey, signature::read_keypair_file, signer::Signer, transaction::VersionedTransaction
};
...
fn bench_simulate(c: &mut Criterion) {
  ...
  for protocol in ["Meteora DLMM", "Raydium CLMM", "Whirlpool", "Pump.fun Amm"] {
    ...
    let (txn, instructions, txn_accounts, instruction_accounts) = runtime.block_on(async {
      let quote = get_api_quote(protocol.to_string(), WSOL_MINT, USDC_MINT, SWAP_AMOUNT).await;
      ...
      let mut instruction_accounts = load_accounts(
        ...
        instructions
          .iter()
          .map(|instruction| instruction.accounts.iter().map(|account| account.pubkey).collect::<Vec<_>>())
          .flatten()
          .collect::<Vec<_>>(),
      )
      .await;
      ...
      if payer_account.is_none() || payer_account.as_ref().unwrap().is_none() || payer_account.as_ref().unwrap().as_ref().unwrap().lamports == 0 {
        let mut payer_account = Account::default();
        payer_account.lamports = SWAP_AMOUNT * 10;

        txn_accounts.insert(
          payer_pubkey,
          Some(payer_account.clone()),
        );
      ...
      (txn, instructions, txn_accounts, instruction_accounts)
    });

    debug!("Loaded accounts: {:#?}", txn_accounts.iter().map(|(pubkey, account)| (pubkey, account.as_ref().map(|account| account.lamports))).collect::<Vec<_>>());

    let mut svm = make_svm();

    group.bench_with_input(BenchmarkId::new("litesvm", protocol), &(txn.clone(), txn_accounts.clone()), |b, (txn, txn_accounts)| {
      b.iter_custom(|n| {
        let mut duration = Duration::ZERO;

        let mut clock = runtime.block_on(async {
    ...
    for program in SVM_PROGRAMS_LIST.iter() {
      ...
      mollusk.add_program(&program, &format!("programs/{program}"), &solana_sdk_ids::bpf_loader_upgradeable::id());
    }
    ...
    group.bench_with_input(
      ...
      |b, (instructions, instruction_accounts)| {
        b.iter_custom(|n| {
          ...
          let clock = runtime.block_on(async {
            let clock = load_clock(&client).await;
            clock
          });
          ...
          for _ in 0..n {
            ...
            let accounts = instruction_accounts
              .into_iter()
              .map(|(pubkey, account)| {
                (*pubkey, account.clone().unwrap_or_default())
              })
              .collect::<Vec<_>>();
    ...
    group.bench_with_input(BenchmarkId::new("rpc", protocol), &txn, |b, txn| {
      b.iter_custom(|n| {
        ...
        for _ in 0..n {
          let start = Instant::now();
          let _ = black_box({
            let result = runtime.block_on(async { client.simulate_transaction(txn).await });
            // warn!("Simulate: {:#?}", result);
            result
          });
          duration += start.elapsed();
...
fn make_svm() -> LiteSVM {
  ...
  feature_set.deactivate(&switch_to_new_elf_parser::id());
...
async fn get_api_quote(protocols: String, input_mint: Pubkey, output_mint: Pubkey, amount_in: u64) -> QuoteResponse {
...
async fn get_lookup_table_remote(client: &RpcClient, lut_address: &Pubkey) -> client_error::Result<AddressLookupTableAccount> {
  let result = client.get_account(lut_address).await?;
  let address_lookup_table = AddressLookupTable::deserialize(&result.data).expect("Couldn't deserialize address lookup table account");
...
async fn txn_accounts_with_client(client: &RpcClient, txn: &VersionedTransaction) -> Vec<AccountMeta> {
  ...
  if let Some(address_table_lookups) = txn.message.address_table_lookups() {
    for lookup_table_addr in address_table_lookups {
      let lookup_table = get_lookup_table_remote(client, &lookup_table_addr.account_key).await.unwrap();
...
async fn txn_accounts_to_list(client: &RpcClient, txn: &VersionedTransaction) -> Vec<Pubkey> {
  ...
  let mut account_keys = account_metas.iter().map(|meta| meta.pubkey).collect::<Vec<_>>();

  if let Some(lookup_tables) = txn.message.address_table_lookups() {
    let luts = lookup_tables.iter().map(|lut| lut.account_key).collect::<Vec<_>>();
...
async fn load_accounts(client: &RpcClient, account_keys: Vec<Pubkey>) -> HashMap<Pubkey, Option<Account>> {
  ...
  debug!("load_accounts_time: {:?}, set accounts time: {:?}", load_accounts_time, set_accounts_time);
